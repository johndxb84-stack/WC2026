...
export async function PUT(request: Request) {
  try {
    ...
    const state = await readState();
    const persistedPredictions = applyManualOverrides(state.predictions);

    if (persistedPredictions.length < merged.length) {
      return NextResponse.json({
        ok: false,
        error: `Redis write verification failed: received ${parsed.length}, attempted ${merged.length}, verified ${persistedPredictions.length}`,
        predictions: persistedPredictions,
        receivedCount: parsed.length,
        storedCount: persistedPredictions.length,
    ...
    return NextResponse.json({ ok: true, predictions: persistedPredictions, receivedCount: parsed.length, storedCount: persistedPredictions.length, resetAt: state.resetAt, persistence: redisPersistenceConfigured() && !redisLastError() ? 'redis' : 'memory' });
  } catch (error) {

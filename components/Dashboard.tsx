...
export function Dashboard() {
  ...
  async function checkThisDevice() {
    ...
    try {
      const response = await fetch('/api/predictions', { cache: 'no-store' });
      if (!response.ok) throw new Error('Could not read Redis');
      const payload = await response.json() as { predictions?: StoredPrediction[]; persistence?: string; env?: { mode?: string } };
      const sharedCount = payload.predictions?.length ?? 0;
      const mode = payload.env?.mode ? ` ${payload.env.mode}` : '';
      setSyncStatus(`Device check: visible ${predictions.length}, local ${currentBackup.predictions.length}, previous ${previousBackup.predictions.length}, Redis ${sharedCount}${payload.persistence === 'redis' ? mode : ' memory'}`);
    } catch {
      setSyncStatus(`Device check: visible ${predictions.length}, local ${currentBackup.predictions.length}, previous ${previousBackup.predictions.length}, Redis unavailable`);
    }
  }
      ...
      <section className="mx-auto max-w-7xl space-y-6">
        <div className="glass rounded-[2rem] p-6 md:p-10">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <button className="liquid-bubble rounded-full px-4 py-2 text-sm font-bold text-white/90" onClick={checkThisDevice} type="button">Check this device</button>

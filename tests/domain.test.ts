...
describe('scoring and settlement', () => {
 // Games before 2026-07-01 keep their original single point values (test fixture kicks off 15 Jun).
 ...
 // Exact score also matches the final after-ET scoreline, not just the 90' one.
 it('awards exact score when the prediction matches the after-ET score', () => expect(scorePrediction({homeScore:1,awayScore:2},{...fixture,homeScore90:1,awayScore90:1,homeScoreExtraTime:1,awayScoreExtraTime:2}).exactScorePoints).toBe(2));
 it('no exact score when the prediction matches neither 90\' nor after-ET score', () => expect(scorePrediction({homeScore:0,awayScore:2},{...fixture,homeScore90:1,awayScore90:1,homeScoreExtraTime:1,awayScoreExtraTime:2}).exactScorePoints).toBe(0));
 it('awards exact score when the AET line matches the FT score of a game with no ET', () => expect(scorePrediction({homeScore:1,awayScore:1,homeScoreExtraTime:2,awayScoreExtraTime:1},{...fixture,homeScore90:2,awayScore90:1}).exactScorePoints).toBe(2));
 it('no exact score from the AET line when the game actually went to ET', () => expect(scorePrediction({homeScore:0,awayScore:0,homeScoreExtraTime:2,awayScoreExtraTime:1},{...fixture,homeScore90:2,awayScore90:1,homeScoreExtraTime:3,awayScoreExtraTime:1}).exactScorePoints).toBe(0));
 ...
 it('awards reached-ET point when the match goes to ET, regardless of the ET score', () => expect(scorePrediction({homeScore:1,awayScore:1,homeScoreExtraTime:1,awayScoreExtraTime:1},{...fixture,homeScore90:1,awayScore90:1,homeScoreExtraTime:3,awayScoreExtraTime:3}).reachedExtraTimePoints).toBe(1));
 ...
 it('awards reached-penalties point when the match goes to a shootout, regardless of the score', () => expect(scorePrediction({homeScore:1,awayScore:1,homePenaltyScore:4,awayPenaltyScore:2},{...fixture,homeScore90:1,awayScore90:1,homePenaltyScore:5,awayPenaltyScore:2}).reachedPenaltiesPoints).toBe(1));
 it('reached-penalties is separate from the exact-shootout point', () => { const s = scorePrediction({homeScore:1,awayScore:1,homePenaltyScore:4,awayPenaltyScore:2},{...fixture,homeScore90:1,awayScore90:1,homePenaltyScore:5,awayPenaltyScore:2}); expect(s.reachedPenaltiesPoints).toBe(1); expect(s.penaltyPoints).toBe(0); });
 it('scores the worked Jean example (90 2-2, ET 3-3, pens 4-2, poss+scorer correct) at 6 on the old system', () => expect(scorePrediction({homeScore:1,awayScore:1,possession:'HOME',firstGoalscorerId:'cody',homeScoreExtraTime:1,awayScoreExtraTime:1,homePenaltyScore:4,awayPenaltyScore:2},{...fixture,homeScore90:2,awayScore90:2,homePossession:60,awayPossession:40,firstGoalscorerId:'cody',homeScoreExtraTime:3,awayScoreExtraTime:3,homePenaltyScore:4,awayPenaltyScore:2}).totalPoints).toBe(6));
 // New rewards: point values double for games kicking off on/after 2026-07-01.
 const newFixture = { id: 'm2', kickoff: new Date('2026-07-04T17:00:00Z') };
 it('doubles the exact-score bundle for games from 1 July', () => expect(scorePrediction({homeScore:2,awayScore:1},{...newFixture,homeScore90:2,awayScore90:1}).totalPoints).toBe(6));
 it('doubles the outcome point for games from 1 July', () => expect(scorePrediction({homeScore:1,awayScore:0},{...newFixture,homeScore90:2,awayScore90:1}).outcomePoints).toBe(2));
 it('reports the multiplier per fixture (old vs new)', () => { expect(scorePrediction({homeScore:0,awayScore:0},{...fixture,homeScore90:0,awayScore90:0}).multiplier).toBe(1); expect(scorePrediction({homeScore:0,awayScore:0},{...newFixture,homeScore90:0,awayScore90:0}).multiplier).toBe(2); });

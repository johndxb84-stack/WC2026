...
describe('prediction rules', () => {
 ...
 it('rejects duplicate score', () => expect(validatePrediction(fixture, order, [{userName:'Nicolas',homeScore:1,awayScore:0,submittedAt:new Date()}], {userName:'Jean',homeScore:1,awayScore:0,submittedAt:new Date('2026-06-15T09:05:00+04:00')}).reason).toMatch(/already been selected/));
...
describe('scoring and settlement', () => {
 ...
 it('extra-time points are disabled in the current scoring rules', () => expect(scorePrediction({homeScore:1,awayScore:1,homeScoreExtraTime:2,awayScoreExtraTime:1},{...fixture,homeScore90:1,awayScore90:1,homeScoreExtraTime:2,awayScoreExtraTime:1}).extraTimePoints).toBe(0));
 it('penalty points are disabled in the current scoring rules', () => expect(scorePrediction({homeScore:1,awayScore:1,homePenaltyScore:5,awayPenaltyScore:4},{...fixture,homeScore90:1,awayScore90:1,homePenaltyScore:5,awayPenaltyScore:4}).penaltyPoints).toBe(0));
